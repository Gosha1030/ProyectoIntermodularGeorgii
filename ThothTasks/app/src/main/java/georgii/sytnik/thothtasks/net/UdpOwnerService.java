...
public class UdpOwnerService extends Service {
    ...
    public static ExternalUserEntity findOrCreateExternalUser(AppDatabase db, byte[] ownerUserId, String ip, int port, String name) {
        ...
        e.externalId = uuidToBytes(UuidV7.newUuid());
    ...
    private void listenLoop(int port) {
        try (DatagramSocket socket = new DatagramSocket(port)) {
            ...
            while (true) {
                ...
                byte[] msgId = hexToBytes(msgIdHex);
    ...
    private void sendAckSmart(String peerIp, int peerPort, JSONObject receivedEnv, String ackOfHex) {
        try {
            ...
            JSONObject ack = MessageCodec.envelope(Protocol.ACK, "owner", System.currentTimeMillis(), ackOfHex, body);
    ...
    private void handle(String peerIp, int peerPort, JSONObject env) throws Exception {
        ...
        switch (type) {
            ...
            case Protocol.SESSION_HELLO: {
                ...
                JSONObject chEnv = MessageCodec.envelope(Protocol.SESSION_CHALLENGE, "owner", System.currentTimeMillis(), env.optString("msgId"), chBody);
                chEnv.put("rid", rid);
            ...
            case Protocol.SESSION_RESULT: {
                ...
                byte[] expectedProof = HmacAuth.mac(kMaster, concat(p.nonceA, p.nonceB, rid.getBytes(StandardCharsets.UTF_8)));
                ...
                String expectedSid = hex(Hkdf.expand(prk, "sid".getBytes(StandardCharsets.UTF_8), 16));
            ...
            case Protocol.ACK: {
                ...
                if (ackOf != null) {
                    db.outboxDao().delete(hexToBytes(ackOf));
                }
                break;
            }
            ...
            case Protocol.VERSION_REQUEST: {
                ...
                if (!checkGranted(peerIp, peerPort, ridHex, resourceIdHex, env.optString("msgId"), body.optString("name", peerIp + ":" + peerPort)))
                    return;
                ...
                JSONObject respEnv = MessageCodec.envelope(Protocol.VERSION_RESPONSE, "owner", System.currentTimeMillis(), env.optString("msgId"), respBody);
            ...
            case Protocol.SCHEDULE_SUMMARY_REQUEST: {
                ...
                JSONObject summary = georgii.sytnik.thothtasks.domain.schedule.ScheduleSummaryBuilder.buildFixedSummary(db, resourceId, startDayUtcMs, days);
                ...
                JSONObject respEnv = MessageCodec.envelope(Protocol.SCHEDULE_SUMMARY_RESPONSE, "owner", System.currentTimeMillis(), env.optString("msgId"), respBody);
                respEnv.put("rid", ridHex);
                respEnv.put("seq", s.nextSeqOut++);
            ...
            case Protocol.SYNC_REQUEST: {
                ...
                JSONObject payload = georgii.sytnik.thothtasks.domain.sync.SyncPayloadBuilder.build(db, resourceId, sinceVersion);
                ...
                for (int i = 0; i < totalChunks; i++) {
                    ...
                    JSONObject chunkEnv = MessageCodec.envelope(Protocol.SYNC_CHUNK, "owner", System.currentTimeMillis(), null, chunkBody);
                ...
                JSONObject doneEnv = MessageCodec.envelope(Protocol.SYNC_DONE, "owner", System.currentTimeMillis(), null, doneBody);
    ...
    private boolean checkGranted(String peerIp, int peerPort, String ridHex, String resourceIdHex, String reqMsgIdHex, String externalName) {
        try {
            ...
            if (ar == null) {
                ...
                ar.requestId = uuidToBytes(UuidV7.newUuid());
    ...
    private void sendHandshakeResult(String ip, int port, String rid, String ackOf, boolean ok, String reason) {
        try {
            ...
            JSONObject resp = MessageCodec.envelope(Protocol.SESSION_RESULT, "owner", System.currentTimeMillis(), ackOf, out);
            resp.put("rid", rid);
    ...
    private void sendReliable(String peerIp, int peerPort, String peerKey, JSONObject env) throws Exception {
        ...
        byte[] msgIdBytes = hexToBytes(msgIdHex);
    ...
    private void ensureAccessChannel() {
        ...
        try {
            ...
            NotificationChannel ch = new NotificationChannel(ACCESS_CHANNEL_ID, "Access requests", NotificationManager.IMPORTANCE_HIGH);
    ...
    private void maybeNotifyAccessOnce(ExternalUserEntity eu, ShareResourceEntity res, String peerIp, int peerPort, String resourceIdHex, String reqMsgIdHex, String externalName) {
        try {
            ...
            String key = hex(eu.externalId) + "|" + resourceIdHex;
            ...
            NotificationCompat.Builder nb = new NotificationCompat.Builder(this, ACCESS_CHANNEL_ID).setSmallIcon(android.R.drawable.ic_dialog_alert).setContentTitle(title).setContentText(text).setStyle(new NotificationCompat.BigTextStyle().bigText(text)).setAutoCancel(true).setPriority(NotificationCompat.PRIORITY_HIGH).addAction(android.R.drawable.checkbox_on_background, "Allow", piAccept).addAction(android.R.drawable.ic_delete, "Reject", piReject).addAction(android.R.drawable.ic_menu_close_clear_cancel, "Block", piBlock);
