...
import georgii.sytnik.thothtasks.domain.sync.Importer;
...
public class UserManagerActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        localAdapter = new SimpleRowAdapter(localRows, new SimpleRowAdapter.Listener() {
            ...
            public void onClick(int position) {
                editLocal(locals.get(position));
            }

            @Override
            public void onDelete(int position) {
                deleteLocal(locals.get(position));
            }
        });
        ...
        sourceAdapter = new ExternalSourceAdapter(sources, new ExternalSourceAdapter.Listener() {
            ...
            @Override
            public void onDelete(ExternalSourceEntity src) {
                deleteSource(src);
            }

            @Override
            public void onCheck(ExternalSourceEntity src) {
                checkUpdate(src);
            }

            @Override
            public void onSync(ExternalSourceEntity src) {
                syncNow(src);
            }
        });
        ...
        btnExternalUserManager.setOnClickListener(v -> startActivity(new Intent(this, ExternalUserManagerActivity.class)));
    ...
    private void load() {
        ...
        new Thread(() -> {
            ...
            for (ShareResourceEntity r : locals) {
                String sub = r.type + " • port=" + (r.port != null ? r.port : "-") + " • active=" + r.active;
    ...
    private void createLocalDialog() {
        ...
        new Thread(() -> {
            ...
            runOnUiThread(() -> {
                ...
                new AlertDialog.Builder(this).setTitle(getString(R.string.btn_add_local)).setView(layout).setPositiveButton(getString(R.string.save), (d, w) -> {
    ...
    private void editLocal(ShareResourceEntity r) {
        ...
        new AlertDialog.Builder(this).setTitle("Edit Local").setView(layout).setPositiveButton(getString(R.string.save), (d, w) -> {
    ...
    private void addSourceDialog() {
        ...
        new AlertDialog.Builder(this).setTitle(getString(R.string.btn_add_source)).setView(layout).setPositiveButton(getString(R.string.save), (d, w) -> {
    ...
    private void addSource(String displayName, String ip, int port, String resourceHex) {
        ...
        ensureNetworkPassword(pwd -> new Thread(() -> {
            try {
                ...
                if (!ar.granted()) {
                    runOnUiThread(() -> Toast.makeText(this, "Acceso denegado: " + ar.reason(), Toast.LENGTH_LONG).show());
                    return;
                }
                ...
                JSONObject summaryBody = georgii.sytnik.thothtasks.net.ScheduleSummaryClientSecure.requestSummarySecure(this, temp, externalName, startDay, 30, 3000);

                boolean conflict = georgii.sytnik.thothtasks.domain.ConflictChecker.hasConflictWithLocal(db, owner.taskRoot, startDay, summaryBody);

                if (conflict) {
                    runOnUiThread(() -> Toast.makeText(this, "Conflicto de horario: no se puede añadir.", Toast.LENGTH_LONG).show());
                    return;
                }

                final android.util.AtomicFile dummy = null;
                ...
                runOnUiThread(() -> {
                    ...
                    tv.setText("Sync: 0/0");
                    androidx.appcompat.app.AlertDialog dlg = new androidx.appcompat.app.AlertDialog.Builder(this).setTitle("Sincronizando…").setView(tv).setCancelable(false).show();
                    dlgRef.set(dlg);
                    tvRef.set(tv);
                });

                SyncClient.SyncResult sr = SyncClient.syncResource(ip, port, resourceHex, externalName, 0, 1200, (received, total) -> runOnUiThread(() -> {
                ...
                Importer.importSyncResult(db, ownerUserId, src, sr.tasks(), sr.taskChanges(), sr.remoteVersion());
    ...
    private void checkUpdate(ExternalSourceEntity src) {
        ensureNetworkPassword(pwd -> new Thread(() -> {
            try {
                ...
                long remoteVersion = VersionClient.requestRemoteVersion(src.ip, src.port, hex(src.resourceId), externalName, 2500);
                ...
                runOnUiThread(() -> Toast.makeText(this, hasUpdate ? "Update disponible" : "Ya actualizado", Toast.LENGTH_SHORT).show());
    ...
    private void syncNow(ExternalSourceEntity src) {
        ensureNetworkPassword(pwd -> new Thread(() -> {
            try {
                ...
                SyncClient.SyncResult sr = SyncClient.syncResource(src.ip, src.port, hex(src.resourceId), externalName, since, 1200, (received, total) -> runOnUiThread(() -> {
                    android.widget.TextView tv = tvRef.get();
                    if (tv != null) tv.setText("Sync: " + received + "/" + total);
                }));
    ...
    private void showPasswordDialog(UserEntity owner, PasswordOkCallback cb) {
        ...
        new AlertDialog.Builder(this).setTitle("Introduce password para habilitar red").setView(et).setPositiveButton("OK", null).setNegativeButton(android.R.string.cancel, null).create();

        AlertDialog dlg = new AlertDialog.Builder(this).setTitle("Introduce password para habilitar red").setView(et).setPositiveButton("OK", null).setNegativeButton(android.R.string.cancel, null).show();
    ...
    private interface PasswordOkCallback {
        void onOk(char[] pwd);
    }
}