...
public class PlacesActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        adapter = new PlaceAdapter(places, new PlaceAdapter.Listener() {
            @Override
            public void onClick(PlaceEntity p) {
                editPlaceDialog(p);
            }

            @Override
            public void onDelete(PlaceEntity p) {
                deletePlace(p);
            }
        });
    ...
    private void createPlaceDialog() {
        ...
        new AlertDialog.Builder(this).setTitle(getString(R.string.create)).setView(etName).setPositiveButton(getString(R.string.save), (d, w) -> {
    ...
    private void editPlaceDialog(PlaceEntity p) {
        ...
        new AlertDialog.Builder(this).setTitle(getString(R.string.save)).setView(etName).setPositiveButton(getString(R.string.save), (d, w) -> {
            String name = etName.getText() != null ? etName.getText().toString().trim() : "";
            if (name.isEmpty()) return;
            updatePlace(p, name);
        }).setNegativeButton(android.R.string.cancel, null).show();
    ...
    private void deletePlace(PlaceEntity p) {
        new AlertDialog.Builder(this).setTitle(getString(R.string.confirm_delete_generic_title)).setMessage(p.placeName).setPositiveButton(R.string.delete, (d, w) -> {
            new Thread(() -> {
